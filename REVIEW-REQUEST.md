...
| 8 | met2 carries an isolated three-ring glyph, connected to nothing (`easter-egg-met2-ring.py`) | Partially re-derived: `easter-egg-ring-connectivity.py` shows 0 met2/via1/via2 contacts (7 met1 wires pass under with no via) and the Morse strip touches nothing on any layer. Still open for you: is the search window (fixed in both scripts) hiding a fourth ring or tail? |
