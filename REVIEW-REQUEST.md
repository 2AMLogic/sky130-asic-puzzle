...
| 7 | Layer 200/0 spells `PER ARENAM AD ASTRA` in Morse (`easter-egg-morse-strip.py`) | Re-derived with independent tooling: `easter-egg-morse-independent.py` parses the GDSII binary with the Python stdlib only (no KLayout) and decodes the same message. Residual check for you: both decoders share the 1:3-width/1-3-7-gap Morse convention — sanity-check that reading against any authoritative Morse spec. |
| 8 | met2 carries an isolated three-ring glyph, connected to nothing (`easter-egg-met2-ring.py`) | Re-derived twice: `easter-egg-ring-connectivity.py` shows 0 met2/via1/via2 contacts (7 met1 wires pass under with no via; strip touches nothing on any layer), and `easter-egg-glyph-survey.py` closes the window caveat — all 1,366 exact-0.30 um met2 tiles on the whole die belong to this one cluster. |
